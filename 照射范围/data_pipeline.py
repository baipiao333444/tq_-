...
# 🛡️ 别名词典：用于拦截 Polymarket 的奇葩缩写，映射回标准城市名
ALIAS_MAP = {
    "NYC": "New York",
    "DC": "Washington",
    "LA": "Los Angeles",
...
DEFAULT_DB = {
    ...
    "Buenos Aires": {"name": "布宜诺斯艾利斯", "lat": -34.8150, "lon": -58.5348, "tz": "America/Argentina/Buenos_Aires",
                     "slug": "buenos-aires"},
    ...
    "Los Angeles": {"name": "洛杉矶", "lat": 33.9416, "lon": -118.4085, "tz": "America/Los_Angeles",
                    "slug": "los-angeles"},
    ...
    "San Francisco": {"name": "旧金山", "lat": 37.6213, "lon": -122.3790, "tz": "America/Los_Angeles",
                      "slug": "san-francisco"},
...
def normalize_temperature(text):
    ...
    if "or higher" in text.lower():
        return "↑" + result_str
    elif "or below" in text.lower():
        return "↓" + result_str
    return result_str
...
def analyze_and_export_weather():
    ...
    try:
        while True:
            ...
            # 如果本页抓到的数量少于 limit，说明到底了
            if len(data) < limit:
                break
                
            offset += limit # 游标往前推 100
            
        print(f"📡 API 深度探测完成：共拉取 {len(all_events)} 个活跃气象节点 (跨越多页)。开始剥离今日数据...")
        ...
        for event in all_events:
            ...
            print(f"SYNC: {db_info['name']} ...", end="\r")
        ...
        with open("today_markets.json", "w", encoding="utf-8") as f:
            json.dump(output_data, f, ensure_ascii=False, indent=2)

        print(f"\n✅ 管道编译完成！今日匹配成功: {len(output_data)} 个城市。")
